...
# another way to read in the csv file in R
preg <- read.table("preg.csv", sep=",", stringsAsFactors = F, header = T)
preg
...
## ------------------------------------------------------------------------
pew %>%
  gather(income, frequency, -religion) #`<$10k`:`$100-150k`
...
IceCream_fixed <- 
  IceCream %>% 
  separate(IceCreamScoops, paste0("scoop", 1:3), sep=",") %>% 
  gather(key= "scoop_order", value="scoop_flavor", scoop1:scoop3 ) %>% 
  # remove rows with NA
  na.omit %>% 
